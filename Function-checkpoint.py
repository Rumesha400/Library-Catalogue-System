...
def plot_author_vs_title(filename):
    try:
        ...
        plt.subplot(1, 3, 1)
        plt.bar(author_counts.index[:10], author_counts.values[:10], color='skyblue')
        plt.xlabel('Author', fontsize=20)
        plt.ylabel('Number of Titles', fontsize=20)
        plt.title('Top 10 Authors by Number of Titles', fontsize=24)
        ...
        plt.axis('equal')
        plt.title('Top 10 Titles by Number of Occurrences', fontsize=24)
